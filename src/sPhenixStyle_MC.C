...
void SetsPhenixStyle (double textsize_)
{
  static TStyle* sphenixStyle = 0;
  std::cout << "sPhenixStyle: Applying nominal settings." << std::endl ;
  if ( sphenixStyle==0 ) sphenixStyle = sPhenixStyle(textsize_);
  gROOT->SetStyle("sPHENIX");
  gROOT->ForceStyle();
}

TStyle* sPhenixStyle_MC(double textsize_) 
{
