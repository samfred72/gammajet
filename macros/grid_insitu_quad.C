...
void grid_insitu_quad() {
  ...
  for (int i = 0; i < ana::nPtBins; i++ ) {
    ...
    TH1D* h = d.get(Form("hratio_%i_1_2_0_0_0",i),1);
    h->Rebin(4);
  ...
  // Trijet reference
  TFile * ftrijet = TFile::Open("/home/samson72/sphnx/gammajet/hists/hists_trijet.root","READ");
  ...
  // Load trees ONCE
  // -----------------------------
  TFile * f = TFile::Open("/home/samson72/sphnx/gammajet/hists/tree_Data.root");
  TTree * t = (TTree*)f->Get("xjtree");
  ...
  TFile * f3 = TFile::Open("/home/samson72/sphnx/gammajet/trees/SAMfile.root","READ");
  ...
  struct Result { float pa, pb, pc, chisq; };
  ...
  int nc = 100; float lowc = -0.0001, highc = 0.0001;
  ...
  for (int ia = 0; ia < na; ia++) {
    ...
    for (int ib = 0; ib < nb; ib++) {
      ...
      for (int ic = 0; ic < nc; ic++) {
        ...
        hchisq->Fill(chisq);
        result.push_back({pa,pb,pc,chisq});
      }
    }
  }
  ...
  for (int i = 0; i < ana::nPtBins; i++) {
    ...
    hstandardinsitu->SetBinError(i+1,  TMath::Sqrt(standardmerrs[i]*standardmerrs[i] + merrp[i]*merrp[i] ));
  }
  ...
  for (int i = 0; i < ana::nTrijetPtBins; i++) {
    ...
    hstandardinsitu3->SetBinError(i+1,  TMath::Sqrt(standardmerrs[ana::nPtBins+i]*standardmerrs[ana::nPtBins+i] + merr3[i]*merr3[i] ));
  }
  ...
  for (int i = result.size() - nKeep; i < result.size(); i++) {
    ...
    TF1 *func = new TF1(Form("tmp%i",i), "pol2", 0, 80);
    func->SetParameters(result.at(i).pa, result.at(i).pb, result.at(i).pc);
    
    for (int j = 0; j < nPoints; ++j) {
      double xi = 80.0/nPoints * j;
  ...
  TF1 * func = new TF1("fbest","pol2",0,80);
  func->SetParameters(minpa,minpb,minpc);
  ...
  TFile * wf = TFile::Open("/home/samson72/sphnx/gammajet/hists/insitu_fit_quad.root","RECREATE");
  func->Write();
  fHigh->Write();
  fLow->Write();
  hinsitu->Write();
