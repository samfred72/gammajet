...
// -----------------------------
void grid_insitu_gammajet(const char * form = "nominal", int ir = 1) {
  ...
  for (int i = 0; i < nTot-1; i++) {
    ...
    cout << "Standard mean for bin " << i << " is " << mean << " / " << (meanp[i]) << endl;
  ...
  const char * wfilename = Form("/home/samson72/sphnx/gammajet/hists/insitu_fit_gammajet_%s_%s.root",form,rname);
