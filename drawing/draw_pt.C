...
void draw_pt() {
  ...
  const char * histname = "hjetetaxj2_2";
  TH1D * hdata = (TH1D*)f->Get(histname);
  TH1D * hmcp = getcombinedhist({f05p,f10p,f20p},{5,10,20},histname,1);
  //TH1D * hmcj = getcombinedhist({f05j,f10j,f20j,f30j,f50j,f70j},{5,10,20,30,50,70},histname,0);
  //hdata->Rebin(4);
  //hmcj->Rebin(4);
  drawMany({hdata,hmcp},{"Data","MC Photon"},"Paired Jet emfrac");
  ...
  //drawMC({f05p,f10p,f20p},{5,10,20},histname,"Leading p_{T}^{reco Jet}",1);
