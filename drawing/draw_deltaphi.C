...
void draw_deltaphi() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    hh[i] = dh.combineMC(Form("hdeltaphi%i_%i",ptbins[i],ir),1);

    hd[i]->SetLineColor(kBlue);
    hp[i]->SetLineColor(kMagenta+1);
    hh[i]->SetLineColor(kGreen);
    hd[i]->SetLineWidth(2);
    hp[i]->SetLineWidth(2);
    hh[i]->SetLineWidth(2);
    hd[i]->Scale(1.0/hd[i]->Integral(),"width");
    hp[i]->Scale(1.0/hp[i]->Integral(),"width");
    hh[i]->Scale(1.0/hh[i]->Integral(),"width");
    
    //d.scale(hd[i],0,M_PI);
    //d.scale(hp[i],0,M_PI);
    hd[i]->GetYaxis()->SetRangeUser(0,5.5);//hd[i]->GetMaximum()*1.2);
