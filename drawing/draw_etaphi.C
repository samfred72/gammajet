...
TH2D * getcombinedhist(vector<TFile*> files, vector<int> samples, string histname, bool isphoton) {
  ...
  TH2D * thehist = anaclone.combineMC(hists,samples,isphoton);
  return thehist;
}
...
void drawMany(vector<TH1D*> hists, vector<string> labels, string axistitle) {
  ...
  TLegend * l = new TLegend(.4,.35,.75,.55);
  ...
  for (int i = 0; i < hists.size(); i++) {
    ...
    hists.at(i)->GetYaxis()->SetRangeUser(0,hists.at(i)->GetMaximum()*1.3);
...
void drawMC(vector<TFile*> files, vector<int> samples, string histname, string label, string labely, bool isphoton) {
  ...
  thehist->GetYaxis()->SetTitle(labely.c_str());
  thehist->Draw("colz");
  return;
...
void draw_etaphi() {
  ...
  const char * type = "jet";
  int num = 3;
  const char * histname = Form("h%setaphi%i",type,num);
  TH2D * hdata = (TH2D*)f->Get(histname);
  TH2D * hmcp = getcombinedhist({f05p,f10p,f20p},{5,10,20},histname,1);
  
  const char * histname2 = Form("h%seta%i",type,num);
  TH1D * hd = (TH1D*)f->Get(histname2);
  TH1D * h0 = (TH1D*)f05p->Get(histname2);
  TH1D * h1 = (TH1D*)f10p->Get(histname2);
  TH1D * h2 = (TH1D*)f20p->Get(histname2);
  TH1D * hp = anaclone.combineMC({h0,h1,h2},{5,10,20},1);
  //TH1D * hmcj = getcombinedhist({f05j,f10j,f20j,f30j,f50j,f70j},{5,10,20,30,50,70},"hclusterpt",0);
  drawMany({hd,hp},{"Data","MC Photon"},"Leading cluster eta");
  anaclone.drawAll({},{"|vz| < 60",Form("Jet R=0.%i",2*(num+1)),"Analysis cuts"},drawx,drawy,fontsize);

  //drawMC({f05p,f10p,f20p},{5,10,20},"clusteretaphi","Leading cluster eta","Leading cluster phi", 1);
  //anaclone.drawAll({"MC Photon"},{"|vz| < 60","Analysis Cuts"},drawx,drawy,fontsize); 
  hdata->GetXaxis()->SetTitle("");
  hdata->GetYaxis()->SetTitle("Leading cluster phi");
  hmcp->GetXaxis()->SetTitle("Leading cluster eta");
  hmcp->GetYaxis()->SetTitle("");
  TCanvas * c = new TCanvas("c","",1000,800);
  c->Divide(2,1,0,0);
  c->cd(1);
  hdata->Draw("col");
  drawText("Data",.4,.8,1,20);
  c->cd(2);
  hmcp->Draw("col");
  drawText("MC Photon",.4,.8,1,20);
